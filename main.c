...
const char program_name[] = "TVNclient";
...
#define VIDEO_FRAMES_PER_SPRITE_FRAME 3
static SDL_Surface *screen, *anim;
static Sprite sprite;
static SDL_mutex *sprite_mutex;
static int avframes_count = 0;


static int packet_queue_put(PacketQueue *q, AVPacket *pkt);

void DrawPixelRGB(SDL_Overlay *bmp, int x, int y, Uint8 R, Uint8 G, Uint8 B)
{
  int Y = R *  .299 + G *  .587 + B *  .114;
  int U = R * -.169 + G * -.332 + B *  .500 + 128.;
  int V = R *  .500 + G * -.419 + B * -.0813 + 128.;

  /* Draw a single pixel on (x, y) */
  *(bmp->pixels[0] + y * bmp->pitches[0] + x) = Y;
  *(bmp->pixels[1] + y/2 * bmp->pitches[1] + x/2) = V;
  *(bmp->pixels[2] + y/2 * bmp->pitches[2] + x/2) = U; 
}

void DrawPixel(SDL_Overlay *bmp, int x, int y, Uint32 pixel)
{
  Uint8 R,G,B;
  SDL_GetRGB(pixel, anim->format, &R, &G, &B);
  DrawPixelRGB(bmp, x, y, R, G, B);
}
...
void ScaleSurface(SDL_Overlay *video, SDL_Surface *pic, Uint16 Width, Uint16 Height)
{
  if(!pic || !Width || !Height)
        return;

  double  _stretch_factor_x = ((double)(Width)  / (double)(pic->w)),
    _stretch_factor_y = ((double)(Height) / (double)(pic->h));

  // SDL_Surface *_ret = SDL_CreateRGBSurface(Surface->flags, Width, Height, Surface->format->BitsPerPixel,
  // Surface->format->Rmask, Surface->format->Gmask, Surface->format->Bmask, Surface->format->Amask)

  Uint32 x,y, o_x, o_y;
  for(y = 0; y < pic->h; y++) //Run across all Y pixels.
    for(x = 0; x < pic->w; x++) //Run across all X pixels.
	...
	for(o_x = 0; o_x < _stretch_factor_x; ++o_x) //Draw _stretch_factor_x pixels for each X pixel.
	  {
	    Uint32 pixel = ReadPixel(pic, x, y);
	    if (pixel != anim->format->colorkey)
	      DrawPixel(video, (Sint32)(_stretch_factor_x * x) + o_x,
			(Sint32)(_stretch_factor_y * y) + o_y, pixel); 
	  }
...
static void video_image_display(VideoState *is)
{
	...
	SDL_LockMutex(sprite_mutex);

	//	if (anim == NULL)
	//fprintf(stderr, "video_image_display : anim is before loop\n");

	if ((sprite.frames_len > 0) && (anim != NULL))
	  {
	    //q	ScaleSurface(vp->bmp, anim, 42, 42);

	    if ( avframes_count > VIDEO_FRAMES_PER_SPRITE_FRAME)
	      {
		anim = sprite_next(&sprite);
		avframes_count = 0;
	      }
	    else
		avframes_count++;
	    ...
	    if (anim == NULL)
	      fprintf(stderr, "anim is NULL !!\n");

	    int i, j;
	
	    //SDL_LockSurface(anim);
	    for (i=0; i<anim->w; i++)
	      for (j=0; j<anim->h; j++)
		{
		  Uint32 pixel = ReadPixel(anim, i, j);
		  if (pixel != anim->format->colorkey)
		    DrawPixel(vp->bmp, i, j, pixel);
		}
	    //SDL_UnlockSurface(anim);
	  }
	SDL_UnlockMutex(sprite_mutex);
...
static void do_exit(void)
{
    ...
    if (sprite.frames_len > 0)
      sprite_free(&sprite);
    
    SDL_DestroyMutex(sprite_mutex);
...
static void event_loop(void)
{
    ...
    for(;;) {
		...
		sprite_free(&sprite);
	      ...
	      if (sprite.frames_len > 0)
		sprite_free(&sprite);
	      sprite_init(&sprite, "volvo");
	      anim = sprite_next(&sprite);
	      if (anim == NULL)
		fprintf(stderr, "event_loop : anim is NULL!!\n");
	      SDL_UnlockMutex(sprite_mutex);   
	      break;
            case SDLK_8:
	      SDL_LockMutex(sprite_mutex);
...
int main(int argc, char **argv)
{
    ...
    //    anim = IMG_Load("anim.gif");
    //    sprite_init(&sprite, "anim.gif");
    //    anim = sprite_next(&sprite);
    sprite.frames_len = 0;
